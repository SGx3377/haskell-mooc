...
longest :: Ord a => [[a]] -> [a]
longest = foldr longestOfTwo []

longestOfTwo :: Ord a => [a] -> [a] -> [a]
longestOfTwo [] ys = ys
longestOfTwo xs [] = xs
longestOfTwo xs ys
...
incrementKey key pairs = map (\(k, v) -> if k == key then (k, v + 1) else (k, v)) pairs
...
transfer from to amount bank
  | Map.member from bank && Map.member to bank && amount >= 0 && bank Map.! from >= amount =
