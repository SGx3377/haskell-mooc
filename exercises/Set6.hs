...
instance Eq Name where
  (==) (Name a) (Name b) = normalize a == normalize b
    where
...
instance Eq RationalNumber where
  (RationalNumber a b) == (RationalNumber c d) = crossProduct a d == crossProduct b c
    where crossProduct x y = x * y
...
simplify (RationalNumber p q) = RationalNumber (p `div` gcdValue) (q `div` gcdValue)
  where gcdValue = gcd p q
...
instance Num RationalNumber where
  ...
  signum (RationalNumber a b)
    ...
    | a /= 0 && b /= 0 = RationalNumber (signum a) (signum b)
...
data Suit = Club | Spade | Diamond | Heart deriving (Show, Eq)
