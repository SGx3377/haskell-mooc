...
firstHalf s = let n = length s in take (div (n + 1) 2) s
...
capitalize s = unwords [capitalizeFirst w | w <- words s]

capitalizeFirst :: String -> String
capitalizeFirst [] = []
capitalizeFirst (w:ws) = toUpper w : ws
...
(+|+) :: [a] -> [a] -> [a]
x +|+ y = foldr (\lst acc -> if null lst then acc else head lst : acc) [] [x, y]
...
interpreter = interpret 0 0

interpret :: Int -> Int -> [String] -> [String]
interpret _ _ [] = []
interpret x y (cmd:cmds) =
    case words cmd of
        ["left"]  -> evalPrint (x - 1) y cmds
        ["right"] -> evalPrint (x + 1) y cmds
        ["up"]    -> evalPrint x (y + 1) cmds
        ["down"]  -> evalPrint x (y - 1) cmds
        ["printX"] -> show x : interpret x y cmds
        ["printY"] -> show y : interpret x y cmds
        _         -> interpret x y cmds

evalPrint :: Int -> Int -> [String] -> [String]
evalPrint x y cmds = interpret x y cmds
