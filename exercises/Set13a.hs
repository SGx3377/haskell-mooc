...
checkNumber (a, b)
  | any checkNumber' [a, b] = Nothing
  | otherwise               = Just (a, b)
  where
    checkNumber' = any (`elem` ['0'..'9'])
...
checkCapitals (for, sur)
  | all (not . null) [for, sur] && all (isUpper . head) [for, sur] = Just (for, sur)
  | otherwise                                                     = Nothing
...
winner scores player1 player2 = do
  ...
  judge (player1, a) (player2, b)

judge :: (String, Int) -> (String, Int) -> Maybe String
judge (p1, x) (p2, y)
  | x >= y    = Just p1
  | otherwise = Just p2
...
selectSum xs indices = foldr f (Just 0) indices
  where
    f idx acc = do
      val <- safeIndex xs idx
      sumRest <- acc
      return (val + sumRest)
...
countAndLog f = foldM (\acc x -> if f x
                                 then do msg (show x)
                                         return (acc + 1)
                                 else return acc) 0
...
paren ch = modify (\s -> if s == -1 then s else s + if ch == '(' then 1 else -1)
...
count x = modify (Map.toList . Map.insertWith (+) x 1 . Map.fromList)
...
occurrences = foldM (\acc x -> count x >> get >>= return . length) 0

-- References for Q6 https://github.com/dandax123/haskell-mooc-solutions/blob/main/Set13a.hs
