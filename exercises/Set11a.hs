...
readWords 0 = return []
readWords n = getLine >>= \i -> readWords (n-1) >>= \s -> return (sort (i:s))
...
readUntil f = getLine >>= \l ->
    if f l
    then return []
    else readUntil f >>= \s -> return (l:s)
...
countdownPrint n = print n >>= \_ -> countdownPrint (n - 1)
...
whenM :: IO Bool -> IO () -> IO ()
whenM cond op = cond >>= \c -> when c op
...
while :: IO Bool -> IO () -> IO ()
while cond op = cond >>= \c ->
    unless (not c) $ op >> while cond op
...
debug s op = putStrLn s >> op >>= \l -> putStrLn s >> return l