...
data Person = NewPerson String Int
  deriving Show

-- fred is a person whose name is Fred and age is 90
fred :: Person
fred = NewPerson "fred" 90

-- getName returns the name of the person
getName :: Person -> String
getName (NewPerson name _) = name 

-- getAge returns the age of the person
getAge :: Person -> Int
getAge (NewPerson _ age) = age

-- setName takes a person and returns a new person with the name changed
setName :: String -> Person -> Person
setName name (NewPerson _ age) = NewPerson name age 

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age (NewPerson name _) = NewPerson name age
...
tick :: UpDown -> UpDown
tick (Increase a) = (Increase (a+1))
tick (Decrease a) = (Decrease (a-1))
...
rgb Green = [0,1,0]
rgb Blue  = [0,0,1]
rgb (Invert x) = case rgb x of
  [r, g, b] -> [1 - r, 1 - g, 1 - b]
rgb (Mix a b) = case (rgb a, rgb b) of
...
toList kvs = foldr (\(k, v) acc -> (k, v) : acc) [] (toPairs kvs)
  where
    toPairs :: KeyVals k v -> [(k, v)]
    toPairs Empty = []
    toPairs (Pair k v l) = (k, v) : toPairs l



fromList :: [(k,v)] -> KeyVals k v
fromList = foldr (\(k, v) acc -> Pair k v acc) Empty
...
fromNat :: Nat -> Int
fromNat nat = go nat 0
  where
    go Zero acc = acc
    go (PlusOne n) acc = go n (acc + 1)


toNat :: Int -> Maybe Nat
toNat n
  | n < 0     = Nothing
  | otherwise = Just $ foldl (\acc _ -> PlusOne acc) Zero [1..n]
...
prettyPrint bin = go bin ""
  where
    go End acc = acc
    go (O xs) acc = go xs ('0' : acc)
    go (I xs) acc = go xs ('1' : acc)

fromBin :: Bin -> Int
fromBin bin = go bin 0 0
  where
    go End _ acc = acc
    go (O xs) i acc = go xs (i + 1) acc
    go (I xs) i acc = go xs (i + 1) (acc + 2^i)
...
toBin n = buildBin n
  where
    buildBin 0 = End
    buildBin x = if odd x then I (buildBin (x `div` 2)) else O (buildBin (x `div` 2))
